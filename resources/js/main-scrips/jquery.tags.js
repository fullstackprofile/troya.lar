...
(function ($) {
    ...
    $.fn.tagSuggest = function (options) {
        ...
        return this.each(function () {
            ...
            tagsElm.after(tagMatches).attr('autocomplete','off').keypress(handleKeys).keyup(handleKeys).blur(function () {
                if (fromTab == true || suggestionsShow) { // tweak to support tab selection for Opera & IE
                    fromTab = false;
                    tagsElm.focus();
                } else {
                    hideSuggestions();
                    tagsElm.val(tagsElm.val().trim())
                }
                elm.timer = setTimeout(function () {
                    hideSuggestions();
                }, 200);
            ...
            tagMatches = $(tagMatches).hide().click(function (ev) {
